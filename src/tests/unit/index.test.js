...
import { isEntity, isEntityType, isProject } from "../../typeGuards"
...
describe("currentlySelected is Entity[]", () => {
	...
	it("fails with id:number", () => {
		expect([{ type: "point", id: 3 }, { type: "circle", id: "1" }].every(entity => isEntity(entity))).toBe(false)
	})
	it("fails with id:object", () => {
		expect([{ type: "point", id: { type: "circle", id: "1" } }, { type: "circle", id: "1" }].every(entity => isEntity(entity))).toBe(false)
	})
	it("fails when missing type", () => {
		expect([{ type: "point", id: "3" }, { id: "1", typez: "point" }].every(entity => isEntity(entity))).toBe(false)
	})
	it("fails when missing id", () => {
		expect([{ type: "point", notId: "99" }, { type: "circle", id: "1" }].every(entity => isEntity(entity))).toBe(false)
	})
	it("fails on extra properties", () => {
		expect([{ type: "point", id: "a string" }, { type: "circle", id: "1", errantProperty: "string" }].every(entity => isEntity(entity))).toBe(false)
	})
})
