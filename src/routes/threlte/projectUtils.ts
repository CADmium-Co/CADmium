...
export function updateExtrusion(extrusionId: string, sketchId: string, length: number, faceIds: string[]) {
	log("[updateExtrusion]", "typeof extrusionId:", typeof extrusionId, extrusionId)
...
export function newExtrusion() {
	...
	checkWasmMessage(message, false, false)
	sendWasmMessage(message)
	workbenchIsStale.set(true)
}
...
export function deleteEntities(sketchIdx: string, selection: Entity[]) {
	...
	const lineIds = reduceToInts(lines.map((e) => e.id), (id: any) => console.error(`[deleteEntities] line id is not an int: ${id}`))
	const arcIds = reduceToInts(arcs.map((e) => e.id), (id: any) => console.error(`[deleteEntities] arc id is not an int: ${id}`))
	const circleIds = reduceToInts(circles.map((e) => e.id), (id: any) => console.error(`[deleteEntities] circle id is not an int: ${id}`))

	if (notEmpty(lineIds)) deleteLines(
		workbenchIdx,
		sketchIdx,
		lineIds
	)
	if (notEmpty(arcIds)) deleteArcs(
		workbenchIdx,
		sketchIdx,
		arcIds
	)
	if (notEmpty(circleIds)) deleteCircles(
		workbenchIdx,
		sketchIdx,
		circleIds
	)
...
function checkWasmMessage(message: Message, abort = true, logError = true): boolean {
	...
	function logOrAbort() {
		...
		if (abort) throw new Error(`"[projectUtils.ts]" ${error}`)
		return false
	}
}
