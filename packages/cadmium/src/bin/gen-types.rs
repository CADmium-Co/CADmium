...
fn main() {
    ...
    writeln!(output, "// This file is generated by the gen-types binary, do not edit").unwrap();
    writeln!(output, "import {{ Direction, IDType, MessageResult, Mode, Plane, PlaneDescription }} from \"cadmium\"").unwrap();
    writeln!(output, "import {{ sendWasmMessage }} from \"./projectUtils\"").unwrap();
    ...
    for (name, fields) in &all_defs {
        ...
        let iface_params_full = fields.iter().map(|f| f.split(":").next().unwrap().trim()).collect::<Vec<_>>().join(", ");
